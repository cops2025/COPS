...
for sim in range(NUMBER_SIMULATIONS):
    ...
    while episode < NUMBER_EPISODES:
        ...
        if status != 'Optimal':
            print("Optimal policy is {}, skip to the next patient".format(status))
            continue

        # util_methods.update_CONSTRAINT(C1b, C2b) # set the C to Cb for calculating the baseline policy
        util_methods.update_CONSTRAINT(1e5, 1e5)
        ...
        if episode == 0:
            ...
            v1 = max(0, cost1_k[s_idx_init, 0] - CONSTRAINT1)
            v2 = max(0, cost2_k[s_idx_init, 0] - CONSTRAINT2)
            if v1 > 0 or v2 > 0:
                violation_ct += 1
        ...
        print('RUN_NUMBER: {}, Episode {}, s_idx_init= {}, ObjRegt = {:.2f}, Cons1Regt = {:.2f}, Cons2Regt = {:.2f}, Infeas = {}, Infeas in EXLP = {}, Cumu_rewards = {:.2f}, Vio_ct = {}, Time = {:.2f}\n'.format(
              RUN_NUMBER, episode, s_idx_init, ObjRegret2[sim, episode], Con1Regret2[sim, episode], Con2Regret2[sim, episode],
              NUMBER_INFEASIBILITIES[sim, episode], select_baseline_policy_ct, rewards_cumulative[-1], violation_ct, dtime))
        ...
        rewards.append(np.sum(ep_cvdrisk)) # append the total reward for the current episode
        rewards_cumulative.append(np.sum(rewards))
        ...
        if episode != 0 and episode%100== 0:
            ...
            f = open(filename, 'ab')
            pickle.dump([R_est_err, C1_est_err, C2_est_err, min_eign_sbp_list, min_eign_hba1c_list, min_eign_cvd_list, NUMBER_SIMULATIONS, NUMBER_EPISODES, objs, rewards, rewards_cumulative, cons1, cons2, pi_k, NUMBER_INFEASIBILITIES, q_k], f)
            f.close()
            objs = []
            rewards = []
            cons1 = []
