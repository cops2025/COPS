...
# set the small values in P to zero
for s in range(N_STATES):
    for a in range(N_ACTIONS):
        for s2 in range(N_STATES):
            ...
            if value < 0.01:
                P[s][a][s2] = 0.0
                print("Set P[{}, {}, {}] = {} to zero".format(s, a, s2, value))

# because now the sume of probs would be <1 since we removed the smaller probabilities (<0.01)
# we tansform the probs by normalize it to sum to 1
for s in range(N_STATES):
    for a in range(N_ACTIONS):
        prob_sum = sum(P[s][a])
        if prob_sum != 1.0 :
            P[s][a] = [p / prob_sum for p in P[s][a]]
            print("Normalized P[{}, {}] to sum to 1".format(s, a))
